		...
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="140px" class="w-100 SellerForm">
			<el-row>
				<el-col :span="12">
					...
					<el-form-item label="营业时间公告" prop="serveTime">
...
<script>
	export default {
		...
		activated() {
			this.editId = sessionStorage.getItem('happySellerId')
			this.getTypes()
			this.getFormData()
		...
		methods: {
			...
			getFormData() {
				this.$http({
					url: this.$http.adornUrl(`/sale/saleSeller/info/${this.editId}`),
					method: "get",
					params: this.$http.adornParams()
				}).then(({
					data
					...
					if(valid) {
						...
						this.$http({
							url: this.$http.adornUrl(
								`/sale/saleSeller/${!this.dataForm.uuid ? "save" : "update"}`
							),
							method: "post",
